/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/rcl-contest-2021-long/submissions/25703214
 * Submitted at: 2021-09-09 17:09:25
...
ll calcMoney(const vector<vector<P>>& dt, ll2d& data) {
  int mon = 1;
  vector<P> MachinesVec;
  rep(di, dt.size()) {
    if (dt[di].size() == 1) {
      MachinesVec.push_back(dt[di][0]);
      mon -= pow(MachinesVec.size(), 3);
    }
    else if (dt[di].size() == 2) {
      auto itr = find(all(MachinesVec), dt[di][0]);
      MachinesVec[distance(MachinesVec.begin(), itr)] = dt[di][1];
    }
    dsu d(n * n);
    set<P> MachineSet;
    rep(i, MachinesVec.size()) {
    ...
    matchVal.resize(0);
    rep(i, m) {
      if (data[i][0] <= di && di <= data[i][1] && data[i][5] == 0) {
    ...
    rep(i, matchVal.size()) {
      if (MachineSet.count(data2point(matchVal[i]))) {
        data[matchVal[i][6]][5] = 1;
        mon += data[matchVal[i][6]][4] * d.size(point2id(data2point(matchVal[i])));
      }
    }
  }
  matchVal.resize(0);
...
int main() {
  ...
  // cout << "[DEB] " << calcMoney(MaximAns, data) << endl;
  // cout << "[DEB] " << chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - start).count() << endl;
  while (true) {
    if (chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - start).count() > 1800) break;

    random_device rnd;
    mt19937 mt(rnd());
    int toChangeDay = (uniform_int_distribution<int>(0, t - 1))(mt);
    int toChangeI = (uniform_int_distribution<int>(0, n - 1))(mt);
    int toChangeJ = (uniform_int_distribution<int>(0, n - 1))(mt);
    vector<vector<P>> chk(MaximAns.size());
    copy(all(MaximAns), chk.begin());
    if (chk[toChangeDay].size() == 1) {
      chk[toChangeDay][0] = { toChangeI, toChangeJ };
    }
    else if (chk[toChangeDay].size() == 2) {
      for (int d = toChangeDay + 1; d < t; d++) {
        if (chk[toChangeDay][1] == chk[d][0]) {
          chk[d][0] = { toChangeI, toChangeJ };
          break;
        }
      }
      chk[toChangeDay][1] = { toChangeI, toChangeJ };
    }
    ll score = calcMoney(chk, data);
    if (score > maximMoney) {
      maximMoney = score;
      MaximAns = chk;
    }
