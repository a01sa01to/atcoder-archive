/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35057930
 * Submitted at: 2022-09-23 12:28:39
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 4540 ms
 */
...
class State {
  ...
  double calcScore() const { return score + Points.size() + (neighbors.size()) / double(Points.size() - m + 1); }
  bool operator<(const State& s) const { return this->calcScore() < s.calcScore(); }
  bool operator>(const State& s) const { return this->calcScore() > s.calcScore(); }
  State Connect(const int z, const int step) {
    ...
    if (flg) {
      ...
      for (int k = 0; k < n; ++k)
        if (Point(k, j).canConnect(nextState.Grid, nextState.Points)) nextState.neighbors.emplace_back(k, j);
      for (int k = 0; k < n; ++k)
        if (Point(i, k).canConnect(nextState.Grid, nextState.Points)) nextState.neighbors.emplace_back(i, k);
      for (int k = -n; k < n; ++k)
        if (Point(i + k, j + k).canConnect(nextState.Grid, nextState.Points)) nextState.neighbors.emplace_back(i + k, j + k);
      for (int k = -n; k < n; ++k)
        if (Point(i + k, j - k).canConnect(nextState.Grid, nextState.Points)) nextState.neighbors.emplace_back(i + k, j - k);
  ...
  queue<pair<int, int>> q_in, q_out;
  {
    ...
    while (d >= 0) {
      for (int j = d; j < n - d - 1; ++j)
        if (Grid[d][j] == Undefined) q_in.emplace(d, j);  // うえ
      for (int i = d; i < n - d - 1; ++i)
        if (Grid[i][n - d - 1] == Undefined) q_in.emplace(i, n - d - 1);  // みぎ
      for (int j = d; j < n - d - 1; ++j)
        if (Grid[n - d - 1][j] == Undefined) q_in.emplace(n - d - 1, j);  // した
      for (int i = d; i < n - d - 1; ++i)
        if (Grid[i][d] == Undefined) q_in.emplace(i, d);  // ひだり
      --d;
    }
  }
  {
    ...
    while (d <= n / 2) {
      for (int j = d; j < n - d - 1; ++j)
        if (Grid[d][j] == Undefined) q_out.emplace(d, j);  // うえ
      for (int i = d; i < n - d - 1; ++i)
        if (Grid[i][n - d - 1] == Undefined) q_out.emplace(i, n - d - 1);  // みぎ
      for (int j = d; j < n - d - 1; ++j)
        if (Grid[n - d - 1][j] == Undefined) q_out.emplace(n - d - 1, j);  // した
      for (int i = d; i < n - d - 1; ++i)
        if (Grid[i][d] == Undefined) q_out.emplace(i, d);  // ひだり
      ++d;
    }
  }
...
int main() {
  ...
  rep(i, n) rep(j, n) {
    ...
    if (t != 0) {
      Debug(i, j);
      InitialState.neighbors.emplace_back(i, j);
    }
  }
  ...
  while (timer.OK() && !que.empty()) {
    ...
    rep(i, s.neighbors.size()) {
      ...
      if (nextState.score != -1) que.emplace(nextState);
    }

    constexpr int BeamWidth = 100;
    if (que.size() > BeamWidth) {
      priority_queue<State> que2;
      rep(i, BeamWidth) {
        if (!timer.OK()) break;
        que2.emplace(que.top());
        que.pop();
      }
      swap(que, que2);
    }
