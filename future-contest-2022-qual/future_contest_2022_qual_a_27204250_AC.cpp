/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/future-contest-2022-qual/submissions/27204250
 * Submitted at: 2021-11-13 16:04:03
...
struct person {
  ...
  void calcScore(ll1d& task_level, ll deltaDay) {
    ld task_norm = 0;
    rep(i, k) task_norm += task_level[i] * task_level[i];
    task_norm = sqrt(task_norm);
    if (deltaDay != 0 && scores.size() >= 30) {
    ...
    rep(i, k) {
      if (deltaDay / ld(sc_cnt + 1) <= 0.7) {
        at_least[i] = at_least[i] + max(0LL, task_level[i] - at_least[i]) * 0.9;
      }
      else if (task_level[i] < at_least[i]) {
        scores_now.second[i] = at_least[i];
      }
      else {
        scores_now.second[i] = at_least[i] + (task_level[i] - estimate[i]) / sqrt(deltaDay);
      }
    }
      ...
      if (!people[j].doing) {
        ...
        rep(i, n) {
          if (!tasks[i].finished && tasks[i].need.empty()) {
            ...
            else if (pending_idx + 30 > i) {
              ll prev_est = 0, comp_est = 0;
              rep(l, k) {
                prev_est += max(0LL, people[j].estimate[l] - tasks[pending_idx].level[l]);
                comp_est += max(0LL, people[j].estimate[l] - tasks[i].level[l]);
              }
              ld prev_est_cp = prev_est - tasks[pending_idx].children.size() + tasks[pending_idx].norm * 0.95;
              ld comp_est_cp = comp_est - tasks[i].children.size() + tasks[i].norm * 0.95;
              if (prev_est_cp < 1) {
