/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/future-contest-2022-qual/submissions/27137661
 * Submitted at: 2021-11-08 23:16:53
        ...
        rep(i, n) {
          if (!tasks[i].finished && tasks[i].need.empty()) {
            ...
            if (pending_idx == -1) {
              ...
              ld prev_est_cp = prev_est - people[j].score * tasks[pending_idx].children.size() / 5;
              ld comp_est_cp = comp_est - people[j].score * tasks[i].children.size() / 5;
              if (prev_est_cp < 1) {
