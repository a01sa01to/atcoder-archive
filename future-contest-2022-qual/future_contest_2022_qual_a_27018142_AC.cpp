/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/future-contest-2022-qual/submissions/27018142
 * Submitted at: 2021-11-04 14:00:32
...
struct person {
  ...
  void calcScore(ll1d& task_level, ll deltaDay) {
    scores.push_back(vector<pair<ll, bool>>(k));
    if (scores.size() > 50) {
      auto itr = find_if(all(scores), [](auto& a) {
        return !a[0].second;
      });
      if (itr == scores.end()) {
        itr = scores.begin();
    ...
    rep(i, k) {
      scores.back()[i] = { task_level[i] - deltaDay / k, deltaDay == 0 };
    }
    ld sum_est = 0;
    rep(j, k) {
      ll sum_false = 0, cnt_false = 0, max_true = 0;
      rep(i, min(50, int(scores.size()))) {
        if (scores[i][j].second == true) {
          if (max_true < scores[i][j].first) {
            sum_false += max_true;
            cnt_false++;
            max_true = scores[i][j].first;
          }
          else {
            sum_false += scores[i][j].first;
            cnt_false++;
          }
        }
        else {
      ...
      estimate[j] = max_true + (max_true * cnt_false - sum_false) * 10000 / cnt_false / 10000;
      sum_est += pow(ld(estimate[j]), 4);
    }
    score = pow(sum_est, 0.25);
  }

  private:
  vector<vector<pair<ll, bool>>> scores;
};
      ...
      if (!people[j].doing) {
        ...
        rep(i, n) {
          if (!tasks[i].finished && tasks[i].need.empty()) {
            ...
            else if (day >= 200 && pending_idx + 50 > i) {
