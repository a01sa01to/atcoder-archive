/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc283/submissions/37531274
 * Submitted at: 2022-12-25 10:40:07
...
int main() {
  ...
  rep(i, h) {
    rep(bit, 8) {
      ...
      rep(j, w) {
        bool tmp = false;
        tmp |= inRange(i - 1, j) && (Grid[i - 1][j] ^ (bit & 1) == Grid[i][j] ^ (bit >> 1 & 1));
        tmp |= inRange(i, j - 1) && (Grid[i][j - 1] == Grid[i][j]);
        tmp |= inRange(i, j + 1) && (Grid[i][j + 1] == Grid[i][j]);
        tmp |= inRange(i + 1, j) && (Grid[i + 1][j] ^ (bit >> 2 & 1) == Grid[i][j] ^ (bit >> 1 & 1));
        ok &= tmp;
      }
      Debug(i, bit, ok);
      int s = (bit << 1) & 0b111;
