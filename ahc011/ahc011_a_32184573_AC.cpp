/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc011/submissions/32184573
 * Submitted at: 2022-06-04 02:31:21
...
int main() {
  ...
  while (ans.size() < t) {
    ...
    while (!pq.empty()) {
      ...
      // if (nowScore < sc) {
      if (nowScore != sc && (nowScore < sc || rand_prob() <= exp(double(sc - nowScore) / pow(0.9, double(timer.now()) / 3000)))) {
        // cout << "[deb] [" << s << "]" << endl;
        nowScore = sc;
        ans += s;
        priority_queue<tuple<ll, string, int, int>> tmp;
