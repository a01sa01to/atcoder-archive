/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/rcl-contest-2021-long/submissions/25698629
 * Submitted at: 2021-09-09 11:19:39
 * Problem URL: https://atcoder.jp/contests/rcl-contest-2021-long/tasks/rcl_contest_2021_long_a
 * Result: WA
 * Execution Time: 1536 ms
 */
  ...
  while (now < t) {
    ...
    if (matchVal.size() == 0 && isOutput) {
      ...
      if (Machines.size() <= MAX_MACHINES && money >= needMoneyToBuy) {
        ...
        // cout << "[CP 4]" << endl;
        if (isOutput) ans.push_back({ newMachinePoint.first, newMachinePoint.second });
        ...
        // cout << "[CP 7]" << endl;
        if (pos == -1 && isOutput) ans.push_back({});
        else {
          if (isOutput) ans.push_back({ moveMachine.first, moveMachine.second, toMachine.first, toMachine.second });
