/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc271/submissions/35315495
 * Submitted at: 2022-10-01 22:34:28
...
int main() {
  ...
  while (!pq.empty()) {
    ...
    for (auto [c, idx, nxt_v] : Graph[v]) {
      auto itr = lower_bound(e[idx].begin(), e[idx].end(), pl);
      if (itr == e[idx].end()) continue;
      int nxt_pl = *itr;
      if (dist[nxt_v] > cost + c) {
        dist[nxt_v] = cost + c;
