/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35006985
 * Submitted at: 2022-09-20 12:39:56
...
ans_t ans;
constexpr int Step1_ACDist = 2;
inline void connect(const int i, const int j, bool& flg, const int step = false) {
  ...
  query_t v;
  if (!nhk(p)) goto skipped;
  v[0] = p->c_pair();
  if (t == 1) {
    if (!nhk(p->R)) goto skipped;
    if (!nhk(p->D)) goto skipped;
    if (!nhk(p->R->D)) goto skipped;
    if (step == 3) goto skipped;
    if (step == 1) {
      if (p->dist(p->R) >= Step1_ACDist) goto skipped;
      if (p->dist(p->D) >= Step1_ACDist) goto skipped;
      if (p->dist(p->R->D) >= Step1_ACDist) goto skipped;
    }
    v[1] = p->R->c_pair();
    v[2] = p->R->D->c_pair();
    v[3] = p->D->c_pair();
  ...
  if (t == 2) {
    if (!nhk(p->U)) goto skipped;
    if (!nhk(p->R)) goto skipped;
    if (!nhk(p->U->R)) goto skipped;
    if (step == 3) goto skipped;
    if (step == 1) {
      if (p->dist(p->U) >= Step1_ACDist) goto skipped;
      if (p->dist(p->R) >= Step1_ACDist) goto skipped;
      if (p->dist(p->U->R) >= Step1_ACDist) goto skipped;
    }
    v[1] = p->U->c_pair();
    v[2] = p->U->R->c_pair();
    v[3] = p->R->c_pair();
  ...
  if (t == 3) {
    if (!nhk(p->D)) goto skipped;
    if (!nhk(p->L)) goto skipped;
    if (!nhk(p->D->L)) goto skipped;
    if (step == 3) goto skipped;
    if (step == 1) {
      if (p->dist(p->D) >= Step1_ACDist) goto skipped;
      if (p->dist(p->L) >= Step1_ACDist) goto skipped;
      if (p->dist(p->D->L) >= Step1_ACDist) goto skipped;
    }
    v[1] = p->D->c_pair();
    v[2] = p->D->L->c_pair();
    v[3] = p->L->c_pair();
  ...
  if (t == 4) {
    if (!nhk(p->U)) goto skipped;
    if (!nhk(p->L)) goto skipped;
    if (!nhk(p->U->L)) goto skipped;
    if (step == 3) goto skipped;
    if (step == 1) {
      if (p->dist(p->U) >= Step1_ACDist) goto skipped;
      if (p->dist(p->L) >= Step1_ACDist) goto skipped;
      if (p->dist(p->U->L) >= Step1_ACDist) goto skipped;
    }
    v[1] = p->U->c_pair();
    v[2] = p->U->L->c_pair();
    v[3] = p->L->c_pair();
  ...
  if (t == 5) {
    if (!nhk(p->DR)) goto skipped;
    if (!nhk(p->DL)) goto skipped;
    if (!nhk(p->DR->DL)) goto skipped;
    if (step == 2) goto skipped;
    if (step == 1) {
      if (p->dist(p->DR) > Step1_ACDist) goto skipped;
      if (p->dist(p->DL) > Step1_ACDist) goto skipped;
      if (p->dist(p->DR->DL) > Step1_ACDist) goto skipped;
    }
    v[1] = p->DR->c_pair();
    v[2] = p->DR->DL->c_pair();
    v[3] = p->DL->c_pair();
  ...
  if (t == 6) {
    if (!nhk(p->UR)) goto skipped;
    if (!nhk(p->UL)) goto skipped;
    if (!nhk(p->UR->UL)) goto skipped;
    if (step == 2) goto skipped;
    if (step == 1) {
      if (p->dist(p->UR) > Step1_ACDist) goto skipped;
      if (p->dist(p->UL) > Step1_ACDist) goto skipped;
      if (p->dist(p->UR->UL) > Step1_ACDist) goto skipped;
    }
    v[1] = p->UR->c_pair();
    v[2] = p->UR->UL->c_pair();
    v[3] = p->UL->c_pair();
  ...
  if (t == 7) {
    if (!nhk(p->UR)) goto skipped;
    if (!nhk(p->DR)) goto skipped;
    if (!nhk(p->UR->DR)) goto skipped;
    if (step == 2) goto skipped;
    if (step == 1) {
      if (p->dist(p->UR) > Step1_ACDist) goto skipped;
      if (p->dist(p->DR) > Step1_ACDist) goto skipped;
      if (p->dist(p->UR->DR) > Step1_ACDist) goto skipped;
    }
    v[1] = p->UR->c_pair();
    v[2] = p->UR->DR->c_pair();
    v[3] = p->DR->c_pair();
  ...
  if (t == 8) {
    if (!nhk(p->UL)) goto skipped;
    if (!nhk(p->DL)) goto skipped;
    if (!nhk(p->UL->DL)) goto skipped;
    if (step == 2) goto skipped;
    if (step == 1) {
      if (p->dist(p->UL) > Step1_ACDist) goto skipped;
      if (p->dist(p->DL) > Step1_ACDist) goto skipped;
      if (p->dist(p->UL->DL) > Step1_ACDist) goto skipped;
    }
    v[1] = p->UL->c_pair();
    v[2] = p->UL->DL->c_pair();
    v[3] = p->DL->c_pair();
...
int main() {
  ...
  while (true) {
    ...
    bool flg = false;
    ++cnt;
    rep(i, n) rep(j, n) connect(i, j, flg, 1);
    if (!flg) {
      auto no = chrono::system_clock::now();
      int t = chrono::duration_cast<chrono::milliseconds>(now - start).count();
      cerr << "Step1 end at " << t << endl;
      break;
    }
  }
  while (true) {
    ...
    bool flg = false;
    while (d <= n / 2) {
      rep(j, n) connect(d, j, flg, 2);          // うえ
      rep(i, n) connect(i, n - d - 1, flg, 2);  // みぎ
      rep(j, n) connect(n - d - 1, j, flg, 2);  // した
      rep(i, n) connect(i, d, flg, 2);          // ひだり
      ++d;
      if (flg) break;
    }
    if (!flg) {
  ...
  while (true) {
    ...
    bool flg = false;
    while (d <= n / 2) {
      rep(j, n) connect(d, j, flg, 3);          // うえ
      rep(i, n) connect(i, n - d - 1, flg, 3);  // みぎ
      rep(j, n) connect(n - d - 1, j, flg, 3);  // した
      rep(i, n) connect(i, d, flg, 3);          // ひだり
      ++d;
      if (flg) break;
    }
    if (!flg) {
  ...
  while (true) {
    ...
    bool flg = false;
    while (d <= n / 2) {
      rep(j, n) connect(d, j, flg, 4);          // うえ
      rep(i, n) connect(i, n - d - 1, flg, 4);  // みぎ
      rep(j, n) connect(n - d - 1, j, flg, 4);  // した
      rep(i, n) connect(i, d, flg, 4);          // ひだり
      ++d;
      if (flg) break;
    }
    if (!flg) {
