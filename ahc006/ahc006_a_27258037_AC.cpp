/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc006/submissions/27258037
 * Submitted at: 2021-11-14 19:03:37
  ...
  while (true) {
    ...
    for (int i = 0; i < collecting.size() - 1; i++) {
      for (int j = i + 1; j < collecting.size(); j++) {
        vector<int> new_collecting;
        for (int k = 0; k < i; k++) new_collecting.push_back(collecting[k]);
        for (int k = i; k < j; k++) new_collecting.push_back(collecting[i + j - k - 1]);
        for (int k = j; k < collecting.size(); k++) new_collecting.push_back(collecting[k]);
    ...
    for (int i = 0; i < returning.size() - 1; i++) {
      for (int j = i + 1; j < returning.size(); j++) {
        vector<int> new_returning;
        for (int k = 0; k < i; k++) new_returning.push_back(returning[k]);
        for (int k = i; k < j; k++) new_returning.push_back(returning[i + j - k - 1]);
        for (int k = j; k < returning.size(); k++) new_returning.push_back(returning[k]);
