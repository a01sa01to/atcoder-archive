/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/35019774
 * Submitted at: 2022-09-21 01:32:15
...
inline void f(vector<Point>& Points, grid_t& Grid, int step, ans_t& ans) {
  while (true) {
    if (!timer.OK()) break;
    int d = n / 2;
    bool flg = false;
    while (d >= 0) {
      for (int j = d; j < n - d; j++) connect(d, j, flg, step, Grid, Points, ans);          // うえ
      for (int i = d; i < n - d; i++) connect(i, n - d - 1, flg, step, Grid, Points, ans);  // みぎ
      for (int j = d; j < n - d; j++) connect(n - d - 1, j, flg, step, Grid, Points, ans);  // した
      for (int i = d; i < n - d; i++) connect(i, d, flg, step, Grid, Points, ans);          // ひだり
      --d;
      if (flg) break;
    }
    if (!flg) {
...
inline void g(vector<Point>& Points, grid_t& Grid, int step, ans_t& ans) {
  while (true) {
    if (!timer.OK()) break;
    int d = 0;
    bool flg = false;
...
int main() {
  ...
  while (timer.OK()) {
    ...
    do {
      s = ans1.size();
      if (n * n >= m * 18 && cnt >= 3) f(Points1, Grid1, 0b010, ans1);
      g(Points1, Grid1, 0b101, ans1);
      g(Points1, Grid1, 0b011, ans1);
      cnt++;
    } while (s != ans1.size());
    break;
  }
  ...
  while (timer.OK()) {
    ...
    do {
      s = ans2.size();
      if (n * n >= m * 18 && cnt >= 3) f(Points2, Grid2, 0b100, ans2);
      g(Points2, Grid2, 0b011, ans2);
      g(Points2, Grid2, 0b101, ans2);
      cnt++;
    } while (s != ans2.size());
    break;
  }
