/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/future-contest-2022-qual/submissions/27205003
 * Submitted at: 2021-11-13 16:34:04
  ...
  void calcScore(ll1d& task_level, ll deltaDay) {
    ...
    rep(i, k) {
      if (deltaDay / ld(sc_cnt + 1) <= 0.7) {
        at_least[i] = at_least[i] + max(0LL, task_level[i] - at_least[i]) * (1.2 - deltaDay / ld(sc_cnt + 1));
      }
      else if (deltaDay / ld(sc_cnt + 1) > 10) {
        at_least[i] = at_least[i] - deltaDay / ld(sc_cnt + 1) * 0.1;
      }
