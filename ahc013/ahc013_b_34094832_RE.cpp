/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/34094832
 * Submitted at: 2022-08-13 15:49:19
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_b
 * Result: RE
 * Execution Time: 2931 ms
 */
      ...
      for (auto di : dir) {
        ...
        while (true) {
          ...
          if (cp[nxt.x][nxt.y].first == 0) {
            ...
            if (cp[nxt.x][nxt.y].first > 0 && t >= 0 && !d.same(i, t)) candt.push_back(t);
            break;
          }
        }
      }
      if (conn.size() + move.size() + 2 > MaxTurn) break;
      for (int bit = (1 << candt.size()) - 1; bit > 0; bit--) {
        vector<int> cand(0);
        rep(i, candt.size()) if (bit & (1 << i)) cand.push_back(candt[i]);
        bool flag = true;
        rep(cc, cand.size()) {
          if (d.same(i, cand[cc])) flag = false;
          rep(ccc, cc) {
            if (d.same(cand[cc], cand[ccc])) flag = false;
          }
        }
        if (!flag) continue;

        vector<int> ckc(compkindcnt[d.leader(i)]);
        rep(cc, cand.size()) {
          rep(b, k) {
            ckc[b] += compkindcnt[d.leader(cand[cc])][b];
          }
        }
        int delta = 0;
        rep(a, k) {
          delta += Combi2(ckc[a]);
          delta -= Combi2(compkindcnt[d.leader(i)][a]);
          rep(cc, cand.size()) delta -= Combi2(compkindcnt[d.leader(cand[cc])][a]);
        }
        rep(a, k) rep(b, k) {
          if (a >= b) continue;
          delta -= ckc[a] * ckc[b];
          delta += compkindcnt[d.leader(i)][a] * compkindcnt[d.leader(i)][b];
          rep(cc, cand.size()) delta += compkindcnt[d.leader(cand[cc])][a] * compkindcnt[d.leader(cand[cc])][b];
        }
        if (delta > 0) {
          rep(cc, cand.size()) {
            if (comp[i].x == comp[cand[cc]].x) {
              for (int j = comp[i].y + 1; j < comp[cand[cc]].y; ++j) cp[comp[i].x][j] = { -1, -1 };
              for (int j = comp[cand[cc]].y + 1; j < comp[i].y; ++j) cp[comp[i].x][j] = { -1, -1 };
            }
            else if (comp[i].y == comp[cand[cc]].y) {
              for (int j = comp[i].x + 1; j < comp[cand[cc]].x; ++j) cp[j][comp[i].y] = { -1, -1 };
              for (int j = comp[cand[cc]].x + 1; j < comp[i].x; ++j) cp[j][comp[i].y] = { -1, -1 };
            }
            else {
              Debug("err0", comp[i], comp[cand[cc]]);
              continue;
            }
            d.merge(i, cand[cc]);
            conn.push_back({ comp[i], comp[cand[cc]] });
          }
          score += delta;
          compkindcnt[d.leader(i)] = ckc;
        }
      }
    }
    // Debug(tmpcnt);
      ...
      rep(i, comp.size()) {
        ...
        for (auto di : dir) {
          ...
          if (cp[nxt.x][nxt.y].first == -100 - 100 * (!di.y) - cp[com.x][com.y].first) {
            ...
            vector<int> tmp(k, 0);
            rep(b, k) {
              tmp[b] = compkindcnt[d.leader(cp[siten.x][siten.y].second)][b] + compkindcnt[d.leader(i)][b];
            }
