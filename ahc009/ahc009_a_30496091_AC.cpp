/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc009/submissions/30496091
 * Submitted at: 2022-03-27 11:04:32
...
inline string neighbor(string s) {
...
inline ld solveProb(const string& t, const vector<vector<Cell>>& wall, vector<vector<ld>>& prob) {
  ld per = 0;
  rep(turn, t.size()) {
    vector tmp(n, vector<ld>(n, 0));
    ...
    // Debug(turn);
    // Debug(tmp[tx][ty] * (400 - turn));
    per += tmp[tx][ty] * (400 - turn);
    tmp[tx][ty] += prob[tx][ty];
    swap(prob, tmp);
  }
  return per;
...
inline ld calcState(const string& t, const vector<vector<Cell>>& wall) {
  vector prob(n, vector<ld>(n, 0));
  prob[sx][sy] = 1;
  return solveProb(t, wall, prob);
}

inline string solve(int s_x, int s_y, vector<vector<Cell>>& wall) {
...
int main() {
  ...
  {  // Step1
    ...
    while (true) {
      ...
      if (t.size() != 0 && ans.size() + t.size() <= 200) {
        ...
        priority_queue<tuple<ld, int, int>> tpq;
        swap(pq, tpq);
        rep(i, n) rep(j, n) {
          if (i != tx && j != ty) {
  ...
  while (true) {  // Step2
    ...
    // if (chrono::duration_cast<chrono::milliseconds>(now - start).count() > 6000) { break; }
    string t = neighbor(ans);
    ld tmp = calcState(t, wall);
    // cout << "[DEBUG] " << fixed << setprecision(15) << tmp << endl;
    if (tmp > best) {
      best = tmp;
      swap(ans, t);
      ++chd;
