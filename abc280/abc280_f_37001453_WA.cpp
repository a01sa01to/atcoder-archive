/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc280/submissions/37001453
 * Submitted at: 2022-12-04 00:03:02
...
int main() {
  ...
  rep(i, n) {
    ...
    while (!que.empty()) {
      ...
      for (auto next_v : Graph[v]) {
        if (dist[next_v.to] != -1) {
          if (dist[next_v.to] != dist[v] + next_v.cost) d.merge(i, n);
          continue;
        }
        dist[next_v.to] = dist[v] + next_v.cost;
        que.push(next_v.to);
      }
    }
  }
