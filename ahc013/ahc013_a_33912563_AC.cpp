/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/33912563
 * Submitted at: 2022-08-10 11:14:46
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: AC
 * Execution Time: 2747 ms
 */
...
inline int calc_score(const vector<ansquery_t>& move, const vector<ansquery_t>& conn, const vector<vector<int>>& cp) {
  ...
  UnionFind uf;
  for (auto r : conn) {
    uf.unite(r.first.to_pair(), r.second.to_pair());
  }

  vector<pair<int, int>> computers;
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      if (cp[i][j] != 0) computers.emplace_back(i, j);
    }
  }
  ...
  for (int i = 0; i < (int) computers.size(); i++) {
    for (int j = i + 1; j < (int) computers.size(); j++) {
      auto c1 = computers[i];
      auto c2 = computers[j];
      if (uf.find(c1) == uf.find(c2)) { score += (cp[c1.first][c1.second] == cp[c2.first][c2.second]) ? 1 : -1; }
    }
  }
...
int main() {
  ...
  tuple<int, vector<ansquery_t>, vector<ansquery_t>> ans = make_tuple(0, vector<ansquery_t>(), vector<ansquery_t>());

  while (timer.check()) {
    vector cp(n + 1, vector<int>(n + 1, -1));
    rep(i, n) rep(j, n) cp[i][j] = c[i][j];
    vector<Point> comp(0);
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
