/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc008/submissions/29234371
 * Submitted at: 2022-02-12 15:32:20
...
void solve() {
  ...
  rep(i, m) {
    ...
    if (human[i].first.y != (i + 1) * 30 / breakpoint) {
      if (fencePlaced[human[i].first + dirs[Right]] && human[i].first.x < 30) {
        cout << Move[Down];
        human[i].first.x++;
      }
      else if (fencePlaced[human[i].first + dirs[Right]]) {
        cout << Move[Up];
        human[i].first.x--;
      }
      else if (human[i].first.y > (i + 1) * 30 / breakpoint) {
        cout << Move[Left];
        human[i].first.y--;
      }
      else {
