/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc012/submissions/32953804
 * Submitted at: 2022-07-03 15:35:11
 * Problem URL: https://atcoder.jp/contests/ahc012/tasks/ahc012_a
 * Result: AC
 * Execution Time: 2517 ms
 */
...
struct State {
  ...
  State neighbor() {
    ...
    rep(_, cnt) {
      int px = rand_int(-10, 10);
      int py = rand_int(-10, 10);
      int qx = rand_int(px + 1, 1000);
      int qy = rand_int(-1000, 1000);
      ret.now.push_back({ px, py, qx, qy });
    }
    ret.calcScore();
    return ret;
  }
...
int main() {
  ...
  rep(_, k) {
    int px = rand_int(-10, 10);
    int py = rand_int(-10, 10);
    int qx = rand_int(-1000, 1000);
    int qy = rand_int(-1000, 1000);
    best.now.push_back({ px, py, qx, qy });
  }
  Debug("random done");
  best.calcScore();
  Debug("score done");
  ...
  while (true) {
    ...
    if (chrono::duration_cast<chrono::milliseconds>(now - start).count() > 2500) {
      break;
    }
