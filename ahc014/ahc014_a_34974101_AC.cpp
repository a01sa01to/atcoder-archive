/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/34974101
 * Submitted at: 2022-09-18 19:41:59
 * Problem URL: https://atcoder.jp/contests/ahc014/tasks/ahc014_a
 * Result: AC
 * Execution Time: 633 ms
 */
...
inline void connect(const int i, const int j, bool& flg, const bool step1 = false) {
...
int main() {
  ...
  while (true) {
    ...
    bool flg = false;
    ++cnt;
    rep(i, n) rep(j, n) connect(i, j, flg, true);
    if (!flg) {
  ...
  while (true) {
    ...
    bool flg = false;
    while (d <= n / 2) {
      rep(j, n) connect(d, j, flg);          // うえ
      rep(i, n) connect(i, n - d - 1, flg);  // みぎ
      rep(j, n) connect(n - d - 1, j, flg);  // した
      rep(i, n) connect(i, d, flg);          // ひだり
      ++d;
      if (flg) break;
    }
    if (!flg) {
