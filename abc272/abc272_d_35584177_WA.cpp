/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc272/submissions/35584177
 * Submitted at: 2022-10-11 16:39:43
...
int main() {
  ...
  while (!q.empty()) {
    ...
    for (int dx = max(-sqrtM, -x); dx < min(sqrtM + 1, n - x); ++dx) {
      if (dx * dx > m) continue;
      long double dy = sqrt(double(m) - dx * dx);
      if (dy != (int) dy) continue;
