/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/agc058/submissions/34050241
 * Submitted at: 2022-08-14 22:59:22
...
bool checker(vector<int> p, vector<int> ans) {
  Debug(ans);
  rep(i, ans.size()) {
    assert(ans[i] >= 1 && ans[i] <= p.size());
    swap(p[ans[i] - 1], p[ans[i]]);
  }
  assert(ans.size() <= p.size() / 2);
  rep(i, p.size() - 1) {
    if (i % 2 == 0)
      assert(p[i] < p[i + 1]);
    else
