/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc247/submissions/30853057
 * Submitted at: 2022-04-10 21:06:53
...
int main() {
  ...
  rep(i, n) {
    bool f = true, s = true;
    if (st.count(v[i].first)) f = false;
    if (st.count(v[i].second)) s = false;
    rep(j, n) {
      if (j <= i) continue;
