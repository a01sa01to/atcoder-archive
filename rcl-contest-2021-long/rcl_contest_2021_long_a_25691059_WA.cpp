/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/rcl-contest-2021-long/submissions/25691059
 * Submitted at: 2021-09-08 21:56:19
 * Problem URL: https://atcoder.jp/contests/rcl-contest-2021-long/tasks/rcl_contest_2021_long_a
 * Result: WA
 * Execution Time: 624 ms
 */
...
void solve_b500(set<P>& Machines, vector<P>& MachinesVec, const ll& MAX_MACHINES, const bool isOutput, const ll3d& ValP, const ll3d& SumGrid) {
  // cout << "[CP 1]" << endl;
  // 買えるなら買う
...
void solve_a500(ll2d& data, set<P>& Machines, vector<P>& MachinesVec, ll3d& SumGrid, const bool isOutput) {
  // cout << "[DEB] machine: " << Machines.size() << endl;
  ll _now = now;
  now += Machines.size() - 1;
  if (now >= t) now = t - 1;
  const ll dayAfter = now;
  set<P> moved;
  vector<P> movedVec;
chk_sort:
  initSort(data);
  // cout << "[cp1]" << endl;
  P afterBest = data2point(matchVal[0]);
  // if (isOutput) cout << "[DEB] ";
  // if (isOutput)
  //   for (ll1d x : matchVal) { cout << "{" << x[2] << ", " << x[3] << "} "; }
  // if (isOutput) cout << "[DEB] day-" << now << " " << afterBest.first << " " << afterBest.second << endl;
  if (Machines.count(afterBest)) {
    if (!moved.count(afterBest)) {
      moved.insert(afterBest);
      movedVec.push_back(afterBest);
    }
    now--;
    goto chk_sort;
  }
  ...
  while (now <= dayAfter) {
    ...
    rep(i, MachinesVec.size()) {
      ...
      if (!moved.count(tmp)) {
        ...
        else if (SumGrid[dayAfter][moveP.first][moveP.second] - SumGrid[now][moveP.first][moveP.second] < SumGrid[dayAfter][tmp.first][tmp.second] - SumGrid[now][tmp.first][tmp.second]) {
          moveP = tmp;
          movePIdx = i;
        }
      }
    }
    // cout << "[cp4] day-" << now << " " << movedVec.size() << " " << Machines.size() << endl;
    P toP = afterBest;
    rep(i, movedVec.size()) {
      P tmp = movedVec[i];
      // cout << "[DEB] " << tmp.first << " " << tmp.second << endl;
      rep(d, 4) {
        P nextP = add(tmp, direction(1)[d]);
        // cout << "[DEB] " << nextP.first << " " << nextP.second << endl;
        // 今～数日後の累積和が大きい
        if (!moved.count(nextP) && !Machines.count(nextP) && InRange(nextP)) {
          // cout << "[DEB] " << nextP.first << " " << nextP.second << endl;
          // cout << "[DEB1] " << toP.first << " " << toP.second << endl;
          if (toP.first == afterBest.first) {
            toP = nextP;
          }
          else if (SumGrid[dayAfter][nextP.first][nextP.second] - SumGrid[now][nextP.first][nextP.second] < SumGrid[dayAfter][toP.first][toP.second] - SumGrid[now][toP.first][toP.second]) {
            toP = nextP;
          }
        }
      }
    ...
    if (moveP.first == -10) {
      ...
      // cout << "[cp5]" << endl;
      // cout << "[DEB] day-" << now << endl;
      if (isOutput && now != _now) output(moveP, toP);
      else if (now == _now) {
        toMoveBest = moveP;
      }
      // if (isOutput) cout << "[DEB] " << moved.size() << endl;
      moved.insert(toP);
      movedVec.push_back(toP);
      Machines.erase(moveP);
      Machines.insert(toP);
      MachinesVec[movePIdx] = toP;
      appendMoney(MachinesVec, data);
      now++;
      // cout << "[cp6]" << endl;
    }
  }
  if (isOutput) output(toMoveBest, afterBest);
  now--;
}
void solve(ll2d& data, ll3d& SumGrid, ll3d& ValP, const ll& MAX_MACHINES, const bool isOutput = false) {
  ...
  while (now < t) {
    ...
    if (matchVal.size() == 0 && isOutput) {
      ...
      solve_b500(Machines, MachinesVec, MAX_MACHINES, isOutput, ValP, SumGrid);
    }
    else {
      solve_a500(data, Machines, MachinesVec, SumGrid, isOutput);
    }
    appendMoney(MachinesVec, data);
    now++;
  }
...
int main() {
  ...
  for (int MAX_MACHINES = 1; MAX_MACHINES <= 40; MAX_MACHINES++) {
