/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc011/submissions/32138546
 * Submitted at: 2022-06-01 12:57:51
 * Problem URL: https://atcoder.jp/contests/ahc011/tasks/ahc011_a
 * Result: AC
 * Execution Time: 2942 ms
 */
...
inline double calcScore(const string& input) {
  ...
  rep(i, n) rep(j, n) {
    if (bs[i][j]) continue;
    if ((tiles[i][j] & Down) && ((i == n - 1) || !(tiles[i + 1][j] & Up))) ed++;
    if ((tiles[i][j] & Right) && ((j == n - 1) || !(tiles[i][j + 1] & Left))) ed++;
    if ((tiles[i][j] & Up) && ((i == 0) || !(tiles[i - 1][j] & Down))) ed++;
    if ((tiles[i][j] & Left) && ((j == 0) || !(tiles[i][j - 1] & Right))) ed++;
  }
  ...
  if (size == n * n - 1) {
    ...
    return (5e5 * double(size) / (n * n - 1) / d.groups().size() / (double(ed) / 5));
  }
}
