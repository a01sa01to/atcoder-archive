/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc003/submissions/22906302
 * Submitted at: 2021-05-25 14:57:41
...
int main(){ cout << fixed << setprecision(15);
  ...
  loop(b,1000){
    ...
    loop(i,res.size()){
      switch(res[i]){
        case 'U':
          Graph[y][x][0] = min(Graph[y][x][0], point);
          Graph[y-1][x][1] = min(Graph[y-1][x][1], point);
          y--;
          break;
        case 'D':
          Graph[y][x][1] = min(Graph[y][x][1], point);
          Graph[y+1][x][0] = min(Graph[y+1][x][0], point);
          y++;
          break;
        case 'L':
          Graph[y][x][2] = min(Graph[y][x][2], point);
          Graph[y][x-1][3] = min(Graph[y][x-1][3], point);
          x--;
          break;
        case 'R':
          Graph[y][x][3] = min(Graph[y][x][3], point);
          Graph[y][x+1][2] = min(Graph[y][x+1][2], point);
          x++;
          break;
      }
    }
