/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/abc207/submissions/23793704
 * Submitted at: 2021-06-26 22:16:45
  ...
  for(double x=-10;x<=10;x++){
    for(double y=-10;y<=10;y++){
      for(double theta=1;theta<360;theta++){
        ...
        bool can = true;
        for(int i=0;i<n;i++){
          if(abs(tmp[i].first - to[i].first) > 0.2 || abs(tmp[i].second - to[i].second) > 0.2){
            can = false;
          }
        }
        if(can){ allcan = true; }
