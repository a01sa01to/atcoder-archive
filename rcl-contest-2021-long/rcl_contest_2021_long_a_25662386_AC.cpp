/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/rcl-contest-2021-long/submissions/25662386
 * Submitted at: 2021-09-07 11:49:17
...
P Pos1 = { -1, -1 };
// P Pos2 = { -1, -1 };
// const ll MAX_VARIABLE_MACHINE = 2;
const ll MAX_CONSTANT_MACHINE = 5;
const ll MAX_CONSTANT_NEXT_MACHINE = 20;
ll nowMachine = 0;
ll needMoneyToBuy = 1;
    ...
    if (matchVal.size() == 0) {
      ...
      if (Pos1.first == -1) {
        ...
        if (nowMachine <= MAX_CONSTANT_MACHINE + MAX_CONSTANT_NEXT_MACHINE && money >= needMoneyToBuy) {
          ...
          attempt:
            rep(dire, 4) {
              constP = add(cml[nowMachine - MAX_CONSTANT_MACHINE], dir[dire]);
            ...
            constP = maximumDir;
            d.merge(point2id(cml[nowMachine - MAX_CONSTANT_MACHINE]), point2id(constP));
          }
          output(constP);
