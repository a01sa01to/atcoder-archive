/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc014/submissions/34959041
 * Submitted at: 2022-09-18 01:12:59
...
class Point {
  ...
  int canConnect() {
    update();
    // 平行 左上
    if (nchk(D) && nchk(R) && nchk(D->R) && nchk(R->D) && D->R == R->D) {
      return 1;
    }
    // 平行 左下
    if (nchk(U) && nchk(R) && nchk(U->R) && nchk(R->U) && U->R == R->U) {
      return 2;
    }
    // 平行 右上
    if (nchk(D) && nchk(L) && nchk(D->L) && nchk(L->D) && D->L == L->D) {
      return 3;
    }
    // 平行 右下
    if (nchk(U) && nchk(L) && nchk(U->L) && nchk(L->U) && U->L == L->U) {
      return 4;
    }
    // 斜め 上
    if (nchk(DR) && nchk(DL) && nchk(DR->DL) && nchk(DL->DR) && DR->DL == DL->DR) {
      return 5;
    }
    // 斜め 下
    if (nchk(UR) && nchk(UL) && nchk(UR->UL) && nchk(UL->UR) && UR->UL == UL->UR) {
      return 6;
    }
    // 斜め 左
    if (nchk(UR) && nchk(DR) && nchk(UR->DR) && nchk(DR->UR) && UR->DR == DR->UR) {
      return 7;
    }
    // 斜め 右
    if (nchk(UL) && nchk(DL) && nchk(UL->DL) && nchk(DL->UL) && UL->DL == DL->UL) {
      return 8;
    }
    return 0;
  }
  ...
  void update(int d = 0) {
    if (d >= 3) return;
    if (nchk(U)) U->update(d + 1);
    if (nchk(D)) D->update(d + 1);
    if (nchk(L)) L->update(d + 1);
    if (nchk(R)) R->update(d + 1);
    if (nchk(UR)) UR->update(d + 1);
    if (nchk(UL)) UL->update(d + 1);
    if (nchk(DR)) DR->update(d + 1);
    if (nchk(DL)) DL->update(d + 1);
    U = getU();
    D = getD();
    L = getL();
    R = getR();
...
int main() {
  ...
  //   if (chrono::duration_cast<chrono::milliseconds>(now - start).count() > 4900) break;
  rep(i, n) rep(j, n) {
