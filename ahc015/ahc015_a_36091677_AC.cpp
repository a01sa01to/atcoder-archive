/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc015/submissions/36091677
 * Submitted at: 2022-10-30 16:49:44
...
int main() {
  ...
  rep(turn, n * n) {
    ...
    int choice = 0;
    // for (int i = 0; i < 4; i++) {
    auto [g, s] = calcScore(Grid, Dir(flavor[turn]));
    // Debug(g, s);
    // Debug((score - s - 1000), exp((score - s - 1000) / 100.0));
    if (score - s > 1000) {}
    else if (score < s || rand_double(0, 1) > exp((score - s - 1000) / 100.0)) {
      score = s;
      tmp = g;
      choice = flavor[turn];
    }
