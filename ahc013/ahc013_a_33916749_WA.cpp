/*
 * Author: a01sa01to
 * Submission URL: https://atcoder.jp/contests/ahc013/submissions/33916749
 * Submitted at: 2022-08-10 15:36:20
 * Problem URL: https://atcoder.jp/contests/ahc013/tasks/ahc013_a
 * Result: WA
 * Execution Time: 2863 ms
 */
  ...
  while (timer.check()) {
    ...
    rep(i, comp.size()) {
      ...
      for (auto d : dir) {
        ...
        if (cp[nxt.x][nxt.y].first < -100 && cp[nxt.x][nxt.y].first % 100 == -cp[com.x][com.y].first) {
          move.push_back({ com, nxt });
          conn.push_back({ nxt, move[cp[nxt.x][nxt.y].second].second });
